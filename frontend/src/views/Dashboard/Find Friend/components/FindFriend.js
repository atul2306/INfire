        ...
        <div className={style.container1}>
          {user.map((curruser) => {
            ...
            return (
              ...
              <Avatar style={{display:"flex" ,width:"120px",height:"120px",marginTop:"5%"}} src={curruser.local.avatar.url || "/broken-image.jpg"}  />
               
                <span className={style.container3}>{curruser.local.name}</span>
                <div className={style.container4}>
                  <span>{curruser.local.followers.length} Follower</span>
                  <span>{curruser.local.following.length} Following</span>
                </div>
                <span>{curruser.local.posts.length} Post</span>
                <button className={style.button} onClick={()=>followuser(curruser._id)} >
                  <span>{checkfollow(curruser._id)?"following":"Follow" }</span>
                </button>
              </div>
            );
          })}
